...
const CLOSURE_DTORS = (typeof FinalizationRegistry === 'undefined')
    ...
    : new FinalizationRegistry(state => {
        wasm.__wbindgen__0.get(state.dtor)(state.a, state.b)
    });
...
function makeMutClosure(arg0, arg1, dtor, f) {
    ...
    const real = (...args) => {
        ...
        try {
            ...
            if (--state.cnt === 0) {
                wasm.__wbindgen__0.get(state.dtor)(a, state.b);
...
*/
function pow_work_wasm(challenge) {
...
function __wbindgen_closure_wrapper37(arg0, arg1, arg2) {
...
onmessage = async function (data) {
    try {
        if (data.data.buf) {
            var importObject = { './spow-wasm_bg.js': this };
            const wasmModule = await WebAssembly.instantiate(data.data.buf, importObject);
            __wbg_set_wasm(wasmModule.instance.exports);
            postMessage("-ready-");
        }
        if (data.data.challenge) {
            let res = await pow_work_wasm(data.data.challenge);
            postMessage(res);
        }
    }
    catch (error) {
